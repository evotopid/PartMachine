function handle_source_ajax_response ( data ) {
  if (data.type == "fail") {
    // display error
    $("#ajax_errors").html("<h2>"+data.message+"</h2>");
...
function bind_new_source_ajax_success() {
  $("#new_source").bind('ajax:success', function (event, data, status, xhr) {
    handle_source_ajax_response(data);
  });
}

function bind_edit_source_ajax_success() {
  $(".edit_source").bind('ajax:success', function (event, data, status, xhr) {
    handle_source_ajax_response(data);
  });
}
